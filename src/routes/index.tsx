...
import {
  ...
  Check, ChevronRight, CirclePlay, Clock3, Command, Database, FileText, Globe2, Infinity,
...
const storyScenes = [
  ...
  { number: "09", eyebrow: "Compounding growth", title: "Your personal brand becomes an asset.", body: "More visibility. Deeper trust. A repeatable engine for opportunities that keeps working while you do your most important work.", icon: Infinity },
];
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background" onMouseMove={(event) => { mouseX.set(event.clientX / window.innerWidth); mouseY.set(event.clientY / window.innerHeight); }}>
      ...
      <header className="fixed inset-x-0 top-4 z-50 mx-auto flex w-[calc(100%-2rem)] max-w-6xl items-center justify-between rounded-full border border-border bg-background/65 px-3 py-2 shadow-[var(--shadow-ambient)] backdrop-blur-2xl md:top-6 md:px-4">
        <a href="#top" className="flex items-center gap-2 px-2 font-bold tracking-tight"><span className="flex size-8 items-center justify-center rounded-xl bg-primary shadow-[var(--shadow-primary)]"><Infinity className="size-5" /></span>ShareOn</a>
